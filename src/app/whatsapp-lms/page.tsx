...
export default function WhatsAppMicroLMSPage() {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      time: '10:45 AM',
      type: 'text',
      text: 'Welcome to Nexus Learn Omni-Sync WhatsApp Micro-LMS! Send voice notes, math photos, or text quizzes.',
    }
  ]);
  ...
  const handleSendMessage = async (text: string, type: 'text' | 'voice_note' | 'image' = 'text') => {
    ...
    const userMsg: ChatMessage = {
      ...
      type,
      text: type === 'voice_note' ? '??? Voice message (0:14) ? "Explain torque and door hinge analogy"' : type === 'image' ? '?? [Handwritten Note: ? = r ? F sin(?)]' : text,
    };
    ...
    <div style={{ background: '#0b141a', minHeight: '100vh', fontFamily: 'Segoe UI, Helvetica, Arial, sans-serif', color: '#e9edef', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <Link href="/dashboard" style={{ color: '#00a884', textDecoration: 'none', fontSize: 13, fontWeight: 700 }}>
            ? Back to Nexus Dashboard
          </Link>
          <span style={{ color: '#54656f' }}>|</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#00a884', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#fff' }}>
              ?
            </div>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#e9edef', display: 'flex', alignItems: 'center', gap: 6 }}>
                <span>Nexus Learn Micro-LMS</span>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <span style={{ fontSize: 11, padding: '4px 10px', borderRadius: 6, background: 'rgba(0,168,132,0.15)', color: '#00a884', fontWeight: 700 }}>
            Tier 2/3 Low-Data Sync Ready
          </span>
          <Link href="/dial-in" style={{ padding: '6px 14px', borderRadius: 8, background: '#00a884', color: '#111b21', textDecoration: 'none', fontSize: 12, fontWeight: 700 }}>
            ?? Try Toll-Free Phone Call ?
          </Link>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', maxWidth: 1200, margin: '0 auto', width: '100%', height: 'calc(100vh - 64px)' }}>
        ...
        <div style={{ width: 340, background: '#111b21', borderRight: '1px solid #222e35', padding: 18, display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto' }}>
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: '#00a884', margin: '0 0 6px' }}>
              Omni-Sync Micro-LMS Engine
            </h3>
            <p style={{ fontSize: 12, color: '#8696a0', margin: 0, lineHeight: 1.5 }}>
              Standard chatbots waste mobile data on text paragraphs. Nexus Learn sends compressed 30s voice notes and mini SVG schematics directly via WhatsApp.
            </p>
          </div>
          ...
          <div style={{ borderTop: '1px solid #222e35', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            >
              <div style={{ fontWeight: 700, fontSize: 13 }}>??? Send Voice Note</div>
              <div style={{ fontSize: 11, color: '#8696a0', marginTop: 2 }}>"Explain torque in physics and why handle is far from hinge"</div>
            </button>
            ...
            >
              <div style={{ fontWeight: 700, fontSize: 13 }}>?? Send Handwritten Math Photo</div>
              <div style={{ fontSize: 11, color: '#8696a0', marginTop: 2 }}>Uploads handwritten equation for Gemini Vision audit</div>
            </button>
            ...
            >
              <div style={{ fontWeight: 700, fontSize: 13 }}>? 2-Minute Daily Quiz Drill</div>
              <div style={{ fontSize: 11, color: '#8696a0', marginTop: 2 }}>Micro-assessment for retention & streaks</div>
            </button>
          </div>
          ...
          <div style={{ marginTop: 'auto', padding: 12, borderRadius: 10, background: 'rgba(0,168,132,0.08)', border: '1px solid rgba(0,168,132,0.2)', fontSize: 11, color: '#8696a0', lineHeight: 1.5 }}>
            <strong style={{ color: '#00a884' }}>Production Webhook:</strong> Connected to Next.js Edge route <code style={{ color: '#fff' }}>/api/whatsapp-gateway</code> for Twilio & WhatsApp Business API webhooks.
          </div>
        </div>
          ...
          <div style={{ flex: 1, padding: 20, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {messages.map(m => (
              ...
              >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
                    {m.quizOptions.map((opt, i) => (
                      <button
                        key={i}
                        onClick={() => {
                          alert(opt.correct ? '?? Correct! When ? = 0?, sin(0?) = 0, so Torque is ZERO (no rotation).' : '? Incorrect. Pushing along the arm produces 0 torque.');
                        }}
                        style={{
                ...
                <div style={{ alignSelf: 'flex-end', fontSize: 10, color: '#8696a0' }}>{m.time}</div>
              </div>
            ))}

            {isTyping && (
              <div style={{ alignSelf: 'flex-start', padding: '8px 14px', borderRadius: 10, background: '#202c33', color: '#00a884', fontSize: 12 }}>
                ? Nexus AI is generating voice note & SVG diagram...
              </div>
            )}
            <div ref={chatEndRef} />
          </div>
          ...
          <div style={{ padding: '12px 18px', background: '#202c33', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button
              ...
              style={{
                ...
                color: '#111b21', fontWeight: 700, cursor: 'pointer', fontSize: 13,
              }}
            >
              Send
            </button>
